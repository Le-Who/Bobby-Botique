...
async def authorize_user(user_id: int) -> None:
    ...
    await db.db_query(
        "INSERT INTO public.users (user_id, is_authorized) VALUES ($1, 1) "
...
async def revoke_user(user_id: int) -> None:
    ...
    await db.db_query("UPDATE public.users SET is_authorized = 0 WHERE user_id = $1", (user_id,))


async def list_authorized_users() -> list[int]:
    """Returns all authorized user IDs."""
    rows = await db.db_query("SELECT user_id FROM public.users WHERE is_authorized = 1")
...
async def get_all_gemini_keys() -> list[dict[str, Any]]:
    ...
    return await db.db_query("SELECT key_hash, api_key FROM public.api_keys")
