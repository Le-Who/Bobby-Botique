...
async def record_daily_activity(user_id: int) -> dict[str, int]:
    ...
    try:
        ...
        # Upsert today's row with streak, resolving yesterday's streak atomically
        result = await db.db_query(
            ...
            VALUES (
                ...
                $2,
                0,
                COALESCE((SELECT NULLIF(current_streak, 0) FROM user_metrics WHERE user_id = $1 AND metric_date = $3), 0) + 1,
                COALESCE((SELECT NULLIF(current_streak, 0) FROM user_metrics WHERE user_id = $1 AND metric_date = $3), 0) + 1
            )
