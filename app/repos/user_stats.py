...
async def get_user_activity_summary(user_id: int) -> tuple[int, int, int]:
    """Returns today's request count, total document count, and conversation count for a user in a single query."""
    result = await db.db_query(
        """
        SELECT
            (SELECT request_count FROM user_metrics WHERE user_id = $1 AND metric_date = CURRENT_DATE) as req_count,
    ...
    if result:
        row = result[0]
        return row["req_count"] or 0, row["doc_count"] or 0, row["conv_count"] or 0
    return 0, 0, 0
