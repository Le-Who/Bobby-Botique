...
async def get_user_activity_summary(user_id: int) -> dict[str, int]:
    """Returns user's request, document, and conversation counts in one DB call."""
    query = """
        SELECT
            (SELECT COALESCE(request_count, 0) FROM user_metrics WHERE user_id = $1 AND metric_date = CURRENT_DATE LIMIT 1) as request_count,
