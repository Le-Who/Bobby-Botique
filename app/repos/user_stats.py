...
async def get_user_activity_summary(user_id: int) -> dict[str, int]:
    """Returns aggregated activity counts for a user in a single query."""
    result = await db.db_query(
        """
        SELECT
            (SELECT COALESCE(request_count, 0) FROM user_metrics WHERE user_id = $1 AND metric_date = CURRENT_DATE LIMIT 1) as req_count,
