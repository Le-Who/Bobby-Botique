...
class GeminiProvider(BaseAIProvider):
    ...
    async def _execute_request(
        ...
        try:
            ...
            try:
                prompt_length = sum(
                    (len(part.get("text", "")) if isinstance(part, dict) else (len(part) if isinstance(part, str) else (0 if isinstance(part, (bytes, bytearray)) else len(str(part))))) for item in history for part in (item.get("parts", []) or []) if part is not None
                )
                has_images = any(
