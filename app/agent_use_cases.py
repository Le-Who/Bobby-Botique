...
class AgentRequestUseCase:
    ...
    async def _resolve_gemini_request(self, preferred_model: str, excluded_key_hashes: Optional[Set[str]] = None):
    ...
    async def _resolve_openrouter_request(self, preferred_model: str, excluded_key_hashes: Optional[Set[str]] = None):
        model_mapping = {
        ...
        openrouter_model = preferred_model if "/" in preferred_model else model_mapping.get(preferred_model, settings.OPENROUTER_DEFAULT_MODEL)
        fallback_priority = [
    ...
    async def increment_key_usage(self, key_hash: str, model_name: str, use_openrouter: bool = None):
        if use_openrouter is None:
            use_openrouter = "/" in model_name or get_use_openrouter()
        if use_openrouter:
    ...
    async def get_ai_response_with_key_rotation(
        ...
        for attempt in range(max_key_retries):
            ...
            if response_text and not is_error_message(response_text):
                await self.increment_key_usage(key_data["key_hash"], model_used, use_openrouter)
