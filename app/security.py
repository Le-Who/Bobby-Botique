...
class InputSanitizer:
    ...
    def validate_file_extension(self, filename: str, allowed_types: Optional[List[str]] = None) -> bool:
        """
        ...
        if extension not in allowed_extensions:
            raise InputSanitizationError(f"File extension '{extension}' not allowed. Allowed: {list(allowed_extensions)}")
    ...
    def validate_telegram_message(self, message: Dict[str, Any]) -> Dict[str, Any]:
        ...
        safe_fields = ['message_id', 'date', 'edit_date', 'forward_from', 'reply_to_message']
    ...
    async def check_rate_limit(self, user_id: int) -> bool:
        ...
        async with self._lock:
            ...
            if len(user_requests) >= self.max_requests:
                logging.warning(f"Rate limit exceeded for user {user_id}: {len(user_requests)}/{self.max_requests} requests")
                return False
