    ...
    *,
    ...
    if history:
        ...
        long_responses = sum(
            ...
            if any((len(p.get("text", "")) if isinstance(p, dict) else len(p) if isinstance(p, str) else 0 if isinstance(p, (bytes, bytearray)) else len(str(p))) > 2000 for p in h.get("parts", []))
        )
        if long_responses >= 3:
