...
def classify_thinking_level(
    ...
    *,
    ...
    if history:
        ...
        def _part_len(p: Any) -> int:
            if p is None or isinstance(p, (bytes, bytearray)) or type(p).__name__ in ("Image", "TaggedImage"):
                return 0
            return len(str(p.get("text", "") if isinstance(p, dict) else p))

        long_responses = sum(1 for h in recent_model_msgs if any(_part_len(p) > 2000 for p in h.get("parts", [])))
        if long_responses >= 3:
