...
async def key_status_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for key_row in all_keys:
        ...
        usage_data = db.db_query("SELECT model_name, request_count FROM key_usage WHERE key_hash = ? AND usage_date = ?", (key_row['key_hash'], today_pacific))
        if not usage_data:
...
async def credits_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for key_row in all_keys:
        display_name = format_key_for_display(key_row['api_key'])
        usage = db.db_query("SELECT credit_usage FROM tavily_key_usage WHERE key_hash = ? AND usage_month = ?", (key_row['key_hash'], current_month))
...
async def list_models_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not db.is_admin(update.effective_user.id): return
    key_data = db.get_available_gemini_key(config.DEFAULT_MODEL)
    if not key_data:
...
async def add_user_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        db.db_query("INSERT INTO users (user_id, is_authorized) VALUES (?, 1) ON CONFLICT (user_id) DO UPDATE SET is_authorized = 1", (user_to_add,))
...
async def del_user_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if user_to_del == config.ADMIN_ID:
            ...
            return
        db.db_query("UPDATE users SET is_authorized = 0 WHERE user_id = ?", (user_to_del,))
