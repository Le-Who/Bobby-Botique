...
async def _resolve_openrouter_request(
    ...
    if "/" in preferred_model:
        ...
        openrouter_model = model_mapping.get(
            preferred_model, settings.OPENROUTER_DEFAULT_MODEL
        )
...
async def _get_ai_response(
    ...
    if use_openrouter:
        logging.info(
            f"🔍 _get_ai_response: routing to OpenRouter, model={model_name}, system_instruction={'provided' if system_instruction else 'None'}, length={len(system_instruction) if system_instruction else 0}"
        )
        ...
        logging.info(
            f"🔍 _get_ai_response: routing to Gemini, model={model_name}, system_instruction={'provided' if system_instruction else 'None'}, length={len(system_instruction) if system_instruction else 0}"
        )
...
async def _get_ai_response_with_key_rotation(
    ...
    logging.error(
        f"All {max_key_retries} key attempts failed for model {preferred_model}"
    )
    is_openrouter = (
...
async def _handle_research_agent(
    ...
    try:
        ...
        await placeholder_message.edit_text(
            f"🧠 Синтезирую ответ на основе {count} символов..."
        )
    except Exception as edit_error:
        logging.error(f"Could not edit placeholder message: {edit_error}")
...
Ответь на вопрос пользователя, основываясь на содержимом документа. Если в документе нет информации для ответа, честно скажи об этом."""
        ...
        gemini_key, model_used, _ = await _resolve_gemini_request(
            settings.DEFAULT_MODEL
        )
        if not gemini_key:
            try:
                await placeholder_message.edit_text(
                    f"🚫 Ключи для модели {settings.DEFAULT_MODEL} закончились."
                )
            except Exception as edit_error:
                logging.error(f"Could not edit placeholder message: {edit_error}")
                # Fallback на новое сообщение
        ...
        response_text, _ = await services.get_gemini_response(
            gemini_key["api_key"], [{"role": "user", "parts": parts}], model_used
        )
...
async def _handle_photo(
    ...
    if resolution:
        try:
            await placeholder_message.edit_text(
                f"🚫 Ключи для модели {chat_state.model} для обработки фото закончились."
            )
        except Exception as edit_error:
            logging.error(f"Could not edit placeholder message: {edit_error}")
            # Fallback на новое сообщение
...
async def _handle_complex_agent_search(
    ...
    if resolution:
        try:
            await placeholder_message.edit_text(
                f"🚫 Ключи для модели {vision_model} (анализ фото) закончились."
            )
        except Exception as edit_error:
            logging.error(f"Could not edit placeholder message: {edit_error}")
        return
    ...
    search_query, _ = await services.get_gemini_response(
        gemini_key["api_key"], [{"role": "user", "parts": parts}], vision_model
    )
