...
async def get_start_menu_content(chat_state, user_id=None):
    ...
    if user_id:
        try:
            # ⚡ Bolt Optimization: Fetch stats concurrently and use COUNT(*) for documents
            # instead of loading the full document list into memory.
            results = await asyncio.gather(
                get_user_today_request_count(user_id),
                get_user_document_stats(user_id),
                get_conversation_count(user_id),
                return_exceptions=True
            )

            # Extract results, handling potential exceptions from gather
            for r in results:
                if isinstance(r, Exception):
                    logging.warning("Error fetching user stats concurrently: %s", r)

            today_requests = results[0] if not isinstance(results[0], Exception) else 0
            doc_stats = results[1] if not isinstance(results[1], Exception) else {"document_count": 0}
            conv_count = results[2] if not isinstance(results[2], Exception) else 0
