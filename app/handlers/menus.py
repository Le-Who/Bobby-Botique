...
async def get_start_menu_content(chat_state, user_id=None):
    ...
    if user_id:
        try:
            # ⚡ Bolt Optimization: Fetches summary stats in a single atomic database query
            # instead of using sequential selects with asyncio and pulling full document records
            # into memory to count them.
            activity_summary = await get_user_activity_summary(user_id)
            req_count = activity_summary["request_count"]
            doc_count = activity_summary["document_count"]
            conv_count = activity_summary["conversation_count"]
