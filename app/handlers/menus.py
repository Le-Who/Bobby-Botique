...
async def get_start_menu_content(chat_state, user_id=None):
    ...
    if user_id:
        try:
            # ⚡ Bolt: Consolidated 3 sequential queries into 1 atomic query to reduce network roundtrips
            activity = await get_user_activity_summary(user_id)
            req_count = activity["request_count"]
            doc_count = activity["document_count"]
            conv_count = activity["conversation_count"]
