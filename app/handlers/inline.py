...
async def handle_inline_query(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if _HOROSCOPE_PREFIX_RE.match(user_query):
        ...
        if not arg:
            arg = "Овен на сегодня"
        horoscope_init_html = f"✨ <b>Гороскоп: {arg}</b>\n<i>Звезды сходятся…</i>"
        horoscope_keyboard = InlineKeyboardMarkup([[InlineKeyboardButton("⏳ Анализ...", callback_data="inline_noop")]])
        results_horo = [
            InlineQueryResultArticle(
                id="horoscope",
                title=f"✨ Гороскоп: {arg[:40]}",
                description="Астрономически точный прогноз",
                input_message_content=InputTextMessageContent(
    ...
    if _TAROT_PREFIX_RE.match(user_query):
        ...
        desc = "Расклад на 3 карты"
        tarot_init_html = "🔮 <b>Таро</b>\n<i>Тасуем колоду…</i>"
        tarot_keyboard = InlineKeyboardMarkup([[InlineKeyboardButton("⏳ Тасуем...", callback_data="inline_noop")]])
        results_tarot = [
            InlineQueryResultArticle(
                id="tarot",
                title=f"🔮 Таро: {arg[:40]}" if arg else "🔮 Расклад Таро (3 карты)",
                description=desc,
                input_message_content=InputTextMessageContent(
                    message_text=tarot_init_html,
                    parse_mode="HTML",
                ),
...
async def _generate_tarot_inline(
    ...
    import contextlib    import logging    from telegram import InlineKeyboardMarkup    from app.agent_use_cases import AgentRequestUseCase    from app.providers.base import get_provider_for_model    from app.tarot import get_tarot_context    from app.utils.text_format import markdown_to_html
    use_case = AgentRequestUseCase()
    ...
    tarot_ctx, card_names = get_tarot_context(3)
    if not tarot_ctx:
        with contextlib.suppress(Exception):
            await bot.edit_message_text(
                inline_message_id=inline_message_id,
    ...
    try:
        ...
        logging.error("Tarot generation failed: %s", e)
        with contextlib.suppress(Exception):
            await bot.edit_message_text(
                inline_message_id=inline_message_id,
