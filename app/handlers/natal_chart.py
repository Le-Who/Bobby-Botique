...
async def natal_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int | str:
    ...
    flow_message = await message.reply_text(
        ...
        "Для натальной карты важны дата, место и точное время рождения; если точного времени нет, "
        "я соберу разбор без домов и асцендента и явно отмечу ограничения.",
...
def _result_caption(report, birth_input: BirthInput) -> str:
    ...
    return (
        ...
        "Полный разбор собран на отдельной странице: визуальная карта, ключевые акценты, подробные секции и расчетные позиции.\n\n"
        f"<b>Фокус:</b> {focus}\n"
        f"{precision_line}"
        "Это бережная навигация по темам, а не приговор. Откройте отчет кнопкой ниже."
    )
