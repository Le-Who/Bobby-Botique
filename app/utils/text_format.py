...
def split_text_safe(text: str, max_length: int = MAX_MESSAGE_LENGTH) -> List[str]:
    ...
    while text:
        ...
        for tag in reversed(open_tags):
            ...
            opening_str = (
                f"<{tag}>" + opening_str
            )  # Simple reconstruction (loses attributes like class="language-python")
            ...
            if tag == "code" or tag == "pre":
                ...
                re.search(f"<{tag}[^>]*>", chunk)  # This finds FIRST, we need LAST.
                # Using rfind logic on string is risky.
                # Let's rely on re.findall
                all_opens = re.findall(f"(<{tag}[^>]*>)", chunk)
                if all_opens:
                    opening_str = all_opens[-1] + opening_str.replace(
                        f"<{tag}>", ""
