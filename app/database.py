...
async def db_query(query: str, params: tuple = (), retries: int = 3):
    ...
    last_exception = None
    # asyncpg uses $1, $2 placeholders, not '?' or '%s'
    query_prepared = query.replace('?', '$').replace('%s', '$')
    for i in range(1, query_prepared.count('$') + 1):
...
async def init_db():
    ...
    try:
        ...
        logging.info(f"Schema migration not needed or already applied.")
        pass

    await db_query("INSERT INTO users (user_id, is_authorized) VALUES ($1, 1) ON CONFLICT (user_id) DO NOTHING", (config.ADMIN_ID,))
