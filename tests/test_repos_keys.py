...
def mock_deps():
    ...
    with patch.object(keys, "db_query", new_callable=AsyncMock) as m_query, \
...
class TestGetAvailableGeminiKey:
    ...
    async def test_returns_cached_key(self, mock_deps):
        ...
        mock_deps["mgr"]._active_keys_cache = {
            "test-model": {"key_hash": "abc", "api_key": "AIza..."}
        }
...
class TestIncrementGeminiKeyUsage:
    ...
    async def test_calls_upsert(self, mock_deps):
        ...
        with patch.object(keys_mod, "_gemini_km") as mock_km, \
