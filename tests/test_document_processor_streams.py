...
def mock_dependencies():
    ...
    # Patch sys.modules
    with patch.dict(
        sys.modules, {"app.database": mock_db, "app.metrics": mock_metrics}
    ):
...
async def test_process_pdf_with_streams(document_processor):
    ...
    with patch.object(
        document_processor,
        "_process_pdf_sync",
        wraps=document_processor._process_pdf_sync,
    ) as mock_sync:
        result = await document_processor._process_pdf(
            pdf_bytes, filename, user_id, file_hash
        )

        assert result["success"] is True
        assert result["pages"] == 1

        # Verify _process_pdf_sync was called with SpooledTemporaryFile
        args, _ = mock_sync.call_args
        assert isinstance(args[0], tempfile.SpooledTemporaryFile)
...
async def test_process_word_with_streams(document_processor):
    ...
    with patch.object(
        document_processor,
        "_process_word_sync",
        wraps=document_processor._process_word_sync,
    ) as mock_sync:
        result = await document_processor._process_word(
            docx_bytes, filename, user_id, file_hash
        )

        assert result["success"] is True
        assert result["content"].strip() == "Hello World"

        # Verify _process_word_sync was called with SpooledTemporaryFile
        args, _ = mock_sync.call_args
        assert isinstance(args[0], tempfile.SpooledTemporaryFile)
