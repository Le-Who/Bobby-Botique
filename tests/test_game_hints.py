...
from app.games.judge import _HINTS_AI_STUDIO_MODEL, generate_hints
...
async def test_generate_hints_ignores_fast_lane_exception_and_uses_slower_valid_lane(hint_settings):
    router = _RouterStub(
        {
            _HINTS_AI_STUDIO_MODEL: (0.0, RuntimeError("boom")),
            "opencode-go/glm-5.1": (
                0.05,
    ...
    assert _HINTS_AI_STUDIO_MODEL in router.calls
...
async def test_generate_hints_background_mode_skips_ai_studio_lane(hint_settings):
    ...
    assert _HINTS_AI_STUDIO_MODEL not in router.calls
