...
def test_pick_ocr_model_fallback_2_5():
    """_pick_ocr_model should fall back to gemini-2.5-flash if 3.5 is not available."""
    mock_settings = MagicMock()
...
def test_pick_ocr_model_fallback_3_preview():
    """_pick_ocr_model should fall back to gemini-3-flash-preview if 3.5 and 2.5 are not available."""
    mock_settings = MagicMock()
    mock_settings.AVAILABLE_MODELS = ["gemini-3.1-flash-lite", "gemini-3-flash-preview"]
