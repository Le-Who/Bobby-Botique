...
def test_parse_range_time_table_ru():
    parsed = parse_birth_table(
        """
        Дата рождения: 1995-02-14
        Время рождения: диапазон
        Если диапазон: 06:00-08:00
        Страна рождения: Украина
        Место рождения: Kyiv, Ukraine
        """
    )

    assert parsed.time_precision == TimePrecision.RANGE
    assert parsed.birth_time_range_start == "06:00"
    assert parsed.birth_time_range_end == "08:00"


def test_parse_rejects_overnight_time_range_for_release_one():
    with pytest.raises(BirthInputParseError, match="Диапазон времени"):
        parse_birth_table(
            """
            Дата рождения: 1995-02-14
            Время рождения: диапазон
            Если диапазон: 23:30-01:30
            Страна рождения: Украина
            Место рождения: Kyiv, Ukraine
            """
        )
...
def test_parse_rejects_incomplete_time_range():
    with pytest.raises(BirthInputParseError, match="Диапазон времени"):
        parse_birth_table(
            """
            Дата рождения: 1995-02-14
            Время рождения: диапазон
            Если диапазон: около 06:00
            Страна рождения: Украина
            Место рождения: Kyiv, Ukraine
            """
        )
...
def test_parse_table_accepts_common_russian_birth_country_name():
    parsed = parse_birth_table(
        """
        Дата рождения: 1995-02-14
        Время рождения: неизвестно
    ...
    assert parsed.birth_place_country_code == "FR"
    assert parsed.birth_place == "Париж"
...
def test_parse_requires_birth_country():
    with pytest.raises(BirthInputParseError, match="Страна рождения"):
        parse_birth_table(
            """
            Дата рождения: 1995-02-14
            Время рождения: неизвестно
