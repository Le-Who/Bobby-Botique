...
async def test_hub_view_no_active_role(chat_state, mock_db, run_patched):
    ...
    text, parse_mode, reply_markup = await run_patched(chat_state, mock_db, view_mode="hub")
...
async def test_hub_view_active_custom_role(chat_state, mock_db, run_patched):
    ...
    text, parse_mode, reply_markup = await run_patched(
        ...
        mock_db,
        view_mode="hub",
        custom_roles_full=[{"id": 10, "title": "My Custom Role", "prompt": "Custom prompt from user."}],
        custom_role_count=1,
    )
...
async def test_list_view_system_roles(chat_state, mock_db, run_patched):
    ...
    with patch(
        ...
        text, parse_mode, reply_markup = await run_patched(chat_state, mock_db, view_mode="system_roles")
...
async def test_details_view_system_role(chat_state, mock_db, run_patched):
    ...
    text, parse_mode, reply_markup = await run_patched(chat_state, mock_db, view_mode="role_details", role_key="sys1")
...
async def test_error_conditions(chat_state, mock_db, run_patched):
    ...
    # 2. Unknown page
    text, parse_mode, reply_markup = await run_patched(chat_state, mock_db, view_mode="invalid_page")
