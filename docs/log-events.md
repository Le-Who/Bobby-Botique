...
| `logging.loss_summary`, `logging.file_sink_unavailable`, `logging.sink_failed`, `logging.sink_recovered`, `logging.format_failed` | bounded writer/bootstrap | drop/write-failure count, sink/phase/error type, recovery window | evidence is incomplete/degraded; formatter failures discard the raw record and all-sink recovery reports uncertain deliveries | metadata |
...
| `telegram.message_received`, `telegram.message_edited`, `telegram.inline_query_received`, `telegram.handler_failed`, `telegram.handler_finished` | authenticated handlers/decorators | handler/update kind, admission/auth facts, duration/outcome | handler lifecycle below the update owner; not a second update terminal | authorized chat text is bounded/scrubbed in the protected stream; sensitive commands stay metadata-only |
| `websocket.session_started`, `websocket.authenticated`, `websocket.session_failed`, `websocket.session_finished` | Mini App WebSocket hooks | endpoint, server request/trace, authenticated actor, duration/outcome | one framework-owned scope and terminal per socket; teardown restores context | no initData/query payload |
...
| `provider.local_validation_finished` | OpenAI-compatible adapter | provider/model/key identity, validation code, `transport_started=false` | invalid local payload rejected before HTTP | full key forbidden; suffix/fingerprint required |
