...
Проверено по checkout `f44541a7` на 2026-09-20. Это словарь текущих контрактов;
...
Лимиты одного запуска `AgenticSearch`: страницы учитываются за весь run,
время/накопленные токены проверяются между итерациями. Текущий запрос и
финальный синтез могут превысить пороги; fallback в handler запускает новый run.
Это не общий жёсткий бюджет всех попыток пользовательского research-запроса.

### Daily Readiness

Состояние сохранённых Easy/Hard assets выбранного дня. Read-only readiness
не запускает AI; delivery-ready допускает отсутствие изображения, fully-ready
требует слово, подсказки, image prompt и изображение для обеих сложностей.
Явная admin preparation выполняется фоновым job и обходит автоматическую image quota.

### Operational Evidence

Защищённый поток событий с request/trace/attempt/delivery/job correlation.
Default `LOG_CONTENT_MODE=full` допускает ограниченный очищенный текст в
разрешённых путях; `metadata` исключает текст. Не каждый этап содержит content.
Readiness Grafana/Loki/Alloy и логин Grafana не доказывают прохождение нового
события до поисковой выдачи; потеря логов оставляет причину события неизвестной.
