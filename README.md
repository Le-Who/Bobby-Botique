...
### Vertex AI (Primary Inline Driver + Enterprise Fallback)

When `VERTEX_AI_KEY` and `VERTEX_AI_PROJECT` are set, Vertex AI Express is used as the **primary slot** in the inline race and the primary fast-path word-generation slot for Crocodile. AI Studio keys race alongside as fallback slots. Live Audio remains pinned to AI Studio Gemini keys and does not route through Vertex.
