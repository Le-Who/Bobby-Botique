...
**Production-Ready** (`v2.13.2`). Deployed as a **3-container Docker stack** on a DigitalOcean VPS: Local Telegram Bot API Server (MTProto), Python bot (Quart + Hypercorn + PTB v20+ webhook), and an Alpine-based media cleanup cron. Uses `concurrent_updates(50)` with a Quart webhook handler to decouple update processing from HTTP acknowledgment. Built-in telemetry, circuit breakers, and connection pooling. The testing architecture follows strictly deterministic Arrange-Act-Assert (AAA), achieving a CI-ready 100% pass rate across over 1803 unit and E2E integration tests.
...
- **Crocodile Mini App Game**: 1-on-1 charades game launched from inline mode (`@bot ÐÑÐ¾ÐºÐ¾Ð´Ð¸Ð»`). Player A is shown the hidden word; Player B guesses via a Telegram Mini App WebSocket session. **How to start:** `@bot ÐÑÐ¾ÐºÐ¾Ð´Ð¸Ð»` (random word), `@bot ÐÑÐ¾ÐºÐ¾Ð´Ð¸Ð»:Ð–Ð¸Ð²Ð¾ÑÐ½ÑÐµ` (category), `@bot ÐÑÐ¾ÐºÐ¾Ð´Ð¸Ð»:=custom` (custom word, known only to creator A). A **4-tier semantic judge** evaluates each guess: Levenshtein exact match â’ 24h Redis judgement cache â’ RaceÃ—3 LLM (`gemini-3.1-pro` + `gemini-2.5-flash-lite` fallback) â’ hardcoded no-match fallback. "Midnight Glass" glassmorphism UI features animated feedback cards (ð§/ð¤ /ð”¥/ð¯) and attempt progress bar.
  ...
  - **Semantic Context Injection (Bug 6.3/6.4):** If a creator chooses a custom word (`=ÐºÑÐ¾ÐºÐ¾Ð´Ð¸Ð»`) that already exists in the built-in bank, the system performs a reverse lookup to find its canonical category (e.g. `Ð–Ð¸Ð²Ð¾ÑÐ½ÑÐµ`) and binds it to the game, providing the LLM with deep context to prevent hint hallucination. Truly custom words are presented as `Ñ Ð»Ð¾Ð²Ð¾ Ð¸Ð³ÑÐ¾ÐºÐ° (Ð¿ÑÐ¾Ð¸Ð·Ð²Ð¾Ð»ÑÐ½Ð°Ñ  ÑÐµÐ¼Ð°)`.
