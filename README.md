...
- **Multimodal Processing Pipeline**: Voice messages transcribed via `gemini-3.1-flash-lite` (high thinking budget for ASR quality) with intent-aware routing (`INTENT:CONVERSATIONAL` vs `INTENT:TRANSCRIPTION`). Conversational voice flows through an interactive Transcribe→Confirm→Respond cycle with inline buttons (✅ Confirm / 📝 Edit / ❌ Cancel). Images analyzed with adaptive vision prompts, documents chunked with query-aware relevance scoring. All media types stored as long-term memories via background `submit_retryable()` tasks. Resilient API key rotation on 503/UNAVAILABLE errors (up to 3 keys, 3 retries each) with model-aware key resolution.
