<!-- ENCODING: UTF-8 -->
<!-- This file uses UTF-8 encoding. All emoji characters are standard Unicode codepoints (e.g. 🐊, 🚀, ✅). When editing programmatically, always open and write this file with encoding='utf-8'. Do NOT re-encode as Latin-1 or cp1252. -->
...
The bot provides intelligent conversational abilities within Telegram, augmenting standard LLM replies with real-time internet search capabilities and multimodal processing (voice transcription, image analysis, PDF/DOCX). It actively manages AI API quotas using a key rotation system, model-level cooldowns, and Crocodile-specific backpressure. Gemini AI Studio minute throttles (`429/RESOURCE_EXHAUSTED` with retry hints) now trigger short model cooldowns instead of midnight key bans, while true daily exhaustion still suspends keys until midnight PT. Stores chat histories and extracted semantic concepts in a PostgreSQL database, and exposes an administrative health dashboard.
...
- **Crocodile Mini App Game**: 1-on-1 charades game launched from inline mode (`@bot Крокодил`). Player A is shown the hidden word; Player B guesses via a Telegram Mini App WebSocket session. **How to start:** `@bot Крокодил` (random word), `@bot Крокодил:Животные` (category), `@bot Крокодил:=custom` (custom word, known only to creator A). A **4-tier semantic judge** evaluates each guess: Levenshtein exact match → 24h Redis judgement cache → Race×3 LLM (`gemini-3.1-pro` + `gemini-2.5-flash-lite` fallback) → hardcoded no-match fallback. "Midnight Glass" glassmorphism UI features animated feedback cards (🧊/🤔 /🔥/🎯) and attempt progress bar.
  ...
  - **Daily Delivery Auth Fix (2026-04-22):** Fixed a critical authentication failure where the scheduled "Play" button was built as a plain `url=` link — opening a regular browser instead of Telegram's Mini App viewer, leaving `tg.initData` empty and causing the WebSocket to close with `4003 initData required`. The button now uses a `t.me/BOT/APP?startapp=daily` deep link (matching the classic game pattern), so Telegram opens the page inside the Mini App viewer and injects `initData`. `web_app=WebAppInfo(...)` is still intentionally avoided since Telegram rejects that button type when editing messages that carry `inline_message_id`.
