...
- **Inline Mode (Cross-Chat Bot Interaction)**: Users invoke the bot from **any Telegram chat** by typing `@gemaibotv2 <query>`. The bot instantly returns 3 tone options (📋 Formal, 😊 Friendly, 😏 Sarcastic) as placeholder inline results. Upon selection, a styled "thinking…" message is sent and then **edited in-place** once the response is ready. Powered by `gemini-3.1-flash-lite-preview` with `thinking_level="off"` (minimal reasoning for speed) and real-time context from `tavily_search_agent` (QnA mode). Uses a strict **20s timeout budget** propagated through the entire provider stack (Router→UseCase→Provider) with `max_retries=1` to prevent timeout contention. On failure, shows a **🔄 Повторить** inline retry button (5-min TTL store) for one-tap re-generation. Requires `/setinline` + `/setinlinefeedback` at 100% in BotFather.
...
- **Multimodal Processing Pipeline**: Voice messages transcribed via `gemini-3.1-flash-lite` (high thinking budget for ASR quality) with intent-aware routing (`INTENT:CONVERSATIONAL`, `INTENT:TRANSCRIPTION`, `INTENT:SEARCH`). Features **Smart Voice Auto-Routing** (bypasses manual confirmation UI for low-complexity transcripts) with regex-based fluff tolerance, **Agentic Voice Search** (detects search intent to natively trigger the web research engine), and **Show & Tell** (voice-replies to photos dynamically inject the image into the LLM context). The **Voice Engine 4.1** pipeline powers outbound voice replies using **ElevenLabs TTS** as the primary provider (voice: Charlotte, `stability=0.50`, `similarity_boost=0.80`; request stitching via `previous_text`/`next_text` eliminates prosodic artifacts at sentence boundaries; key-rotation load-balancing across `ELEVENLABS_API_KEYS` pool) with **atomic fallback** to Gemini REST TTS (`gemini-2.5-flash-preview-tts`, voices: Aoede, Rasalgethi) on quota exhaustion. Audio is transcoded into Telegram-compliant PCM→OGG Opus via `ffmpeg` at **24k bitrate** (optimized for speech). The Gemini TTS engine uses **Adaptive Sequential Chunking** (1800 max bytes) to balance intonation consistency while protecting the acoustic generation model from structural memory limits and generative hallucinations. A highly optimized Steerable Voice prompt enforces **Strict Verbatim Constraints** (blocking 500-error hallucinatory fillers like "To be continued") and features **Dynamic Personalities** tied directly to the MiniApp's **Independent TTS Temperature** slider. The system intelligently switches stylistic instructions and pacing tags on the fly mapping to strict news-anchor (`temp <= 0.3`), conversational assistant (`0.4-0.7`), or highly engaging storytelling (`temp >= 0.8`), guaranteeing strict phonetic adherence alongside customizable emotional expression, finished with a low-threshold PCM silence trimming gate (400 amplitude) to strip out zeros without truncating quiet speech. Featuring **Zero-Latency Voice Intent Detection**: if a user requests audio via text ("озвучь ответ"), a custom system instruction prompts the LLM to emit a `[VOICE]` tag. The streaming engine invisibly detects and strips this tag from the first chunk on the fly, triggering the TTS pipeline dynamically without adding secondary API calls or slowing down text delivery. Eradicates "sticky voice" by enforcing **Strict Modality Compliance**: text remains text, voice remains voice, unless explicitly overridden. Users can hit the **Re-transcribe (Flash)** button to retry stubborn transcription with a heavier model (`gemini-3-flash-preview`). All media types stored as long-term memories via background `submit_retryable()` tasks.
...
- **Inline Mode**
  ...
  - _Steps_: From any Telegram chat, user types `@gemaibotv2 <query>`. Bot instantly returns 3 tone variants (placeholder). User selects tone → placeholder sent to chat. Bot receives `ChosenInlineResult`, runs Tavily search + Gemini generation (20s timeout budget, minimal thinking) in background, then edits the placeholder in-place with the final HTML-formatted result. On timeout or failure, a `🔄 Повторить` retry button is shown.
  - _Expected Outcome_: Full AI answer (with fresh web context) appears in the original chat, in-place, within ~10–20 s. On failure, user can retry with one tap.
