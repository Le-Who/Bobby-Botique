...
When `VERTEX_AI_KEY` and `VERTEX_AI_PROJECT` are set, Vertex AI Express is used as the **primary slot** in the inline race, the primary fast-path word-generation slot for Crocodile, the primary background hint-prewarm lane, and the **only** runtime path for Live Audio (`gemini-live-2.5-flash-native-audio`). AI Studio keys still race alongside as fallback slots for inline/chat paths, but Live Audio no longer routes through AI Studio.
