...
- **Image Generation (Imagen 4)**: Text-to-image generation via `/draw <prompt>` (aliases `/img`, `/image`, `/generate`) using Google Imagen 4 models (`imagen-4.0-fast-generate-001`, `imagen-4.0-generate-001`, `imagen-4.0-ultra-generate-001`). Features a **Heartbeat animation** (`ChatAction.UPLOAD_PHOTO` refreshed every 4.5 s) during the 10–20 s generation, and an **Interactive Canvas** inline keyboard for one-tap regeneration, aspect ratio switching (1:1, 3:4, 4:3, 9:16, 16:9), and model switching (Fast/Base/Ultra). An **isolated per-key RPD budget** (`imagen:rpd:<hash>` in Redis, with in-memory fallback) tracks the 25 requests/day free-tier limit independently from LLM and audio keys — Imagen quota exhaustion cannot suspend chat keys. Safety blocks and quota errors surface as actionable user messages.
