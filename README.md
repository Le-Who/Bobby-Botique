...
- **Multimodal Processing Pipeline**: Voice messages transcribed via `gemini-3.1-flash-lite` (high thinking budget for ASR quality) with intent-aware routing (`INTENT:CONVERSATIONAL`, `INTENT:TRANSCRIPTION`, `INTENT:SEARCH`). Features **Smart Voice Auto-Routing** (bypasses manual confirmation UI for low-complexity transcripts) with regex-based fluff tolerance, **Agentic Voice Search** (detects search intent to natively trigger the web research engine), and **Show & Tell** (voice-replies to photos dynamically inject the image into the LLM context). Hardened against ASR instruction-following bleed (where transcribe prompts attempt to answer questions). The **Voice Engine 3.0** pipeline powers outbound voice replies using **REST TTS** (`gemini-2.5-flash-preview-tts`, voice: Aoede) as the sole provider. Features a 9-stage **text pre-processing pipeline** (strips Markdown code blocks/links/headers/bold-italic, URLs, HTML tags, emoji clusters) and a production-grade **Director's Notes** prompt with Russian phonetic intelligence (ё/е diacritics with concrete examples, abbreviation expansion, stress correction, typo recovery from context) and anti-artifact delivery constraints (no breathiness/rasp/commentary injection). Audio is transcoded into Telegram-compliant PCM→OGG Opus via `ffmpeg` at **24k bitrate** (optimized for speech). Long texts are split into **≤1500-char sentence-boundary chunks** and generated **in parallel** (up to 3 concurrent TTS calls) with PCM concatenation — ensuring **100% full-text voicing** regardless of response length. **Proactive key rotation** via `DAILY_LIMITS` prevents quota exhaustion (429 errors) before they happen — keys rotate by usage count, not by failure. Voice reply generation is **fire-and-forget** with priority launch before state persistence. **Atomic Voice-for-Voice toggle**: voice replies are triggered only for voice-sourced messages or explicit text keywords (`"озвучь ответ"`), never persisted across turns. Users can hit the **Re-transcribe (Flash)** button to retry stubborn transcription with a heavier model (`gemini-3-flash-preview`). Includes a hidden developer command (`/asr <model>`) via Reply context for rapid AB testing of ASR model hallucination rates. Pipeline is hardened against silent timeouts, features **synchronous double-tap rejection** via atomic `is_processing` locks, and safely isolates conversational context to specific `message_id`s. Images analyzed with adaptive vision prompts, documents chunked with query-aware relevance scoring. All media types stored as long-term memories via background `submit_retryable()` tasks. Resilient API key rotation on 503/UNAVAILABLE errors and quota exhaustion (e.g., TTS 10 RPD / 3 RPM caps) dynamically spins up backup keys inside the `ProviderRouter`.
