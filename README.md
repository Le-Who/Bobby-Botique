...
- **Inline Mode (Cross-Chat Bot Interaction)**: Users invoke the bot from **any Telegram chat** by typing `@gemaibotv2 <query>`. The bot instantly returns 3 tone options (📋 Formal, 😊 Friendly, 😏 Sarcastic) as placeholder inline results. Upon selection, a styled "thinking…" message is sent and then **edited in-place** once the response is ready. Powered by `gemini-3.1-flash-lite-preview` with real-time context from `tavily_search_agent` (QnA mode). Requires `/setinline` + `/setinlinefeedback` at 100% in BotFather.
...
- **Inline Mode**
  ...
  - _Steps_: From any Telegram chat, user types `@gemaibotv2 <query>`. Bot instantly returns 3 tone variants (placeholder). User selects tone → placeholder sent to chat. Bot receives `ChosenInlineResult`, runs Tavily search + Gemini generation in background, then edits the placeholder in-place with the final HTML-formatted result.
  - _Expected Outcome_: Full AI answer (with fresh web context) appears in the original chat, in-place, within ~10–30 s.
