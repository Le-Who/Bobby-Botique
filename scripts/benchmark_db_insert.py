...
async def benchmark():
    ...
    try:
        ...
        async with pool.acquire() as conn:
            ...
            await conn.execute(
                "INSERT INTO users (user_id, is_authorized) VALUES (999999, 1) ON CONFLICT (user_id) DO NOTHING"
            )
            ...
            # Clean up messages
            await conn.execute(
                "DELETE FROM conversation_messages WHERE conversation_id = $1", conv_id
            )
            ...
            improvement = (
                n_plus_one_duration / batch_duration if batch_duration > 0 else 0
            )
