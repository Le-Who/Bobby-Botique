...
## 2024-05-18 - Missing ARIA States on Dynamic Badges
**Learning:** Dynamic badge updates (like hidden-by-default error counters) are often missed by screen readers because their DOM updates aren't announced.
**Action:** When building or enhancing dynamic badging elements, ensure they have both `aria-live="polite"` and `aria-atomic="true"` attributes so screen readers announce dynamic changes correctly.
