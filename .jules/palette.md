...
## 2024-05-18 - Dynamic UI Badge ARIA Attributes
**Learning:** Found a pattern where dynamic badging elements (like hidden-by-default error counters) lack WAI-ARIA live region properties, causing screen readers to miss the real-time updates when the badge toggles visibility and updates its text content.
**Action:** When building or enhancing dynamic badging elements, ensure they include both `aria-live="polite"` and `aria-atomic="true"` attributes so screen readers correctly and fully announce dynamic changes without unnecessarily interrupting the user.
