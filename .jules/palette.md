...
## 2024-05-23 - Dynamic Badging Requires Live Regions
**Learning:** Found that dynamic notification badges (like the error counter in `dashboard.html`) which are hidden initially and updated via JavaScript are completely ignored by screen readers when they appear or change value.
**Action:** When adding or enhancing dynamic badging elements (especially those that are toggled from `display: none`), always include `aria-live="polite"` and `aria-atomic="true"` so screen readers correctly announce the new values as they update.
