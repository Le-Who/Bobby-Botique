...
## 2025-05-07 - Missing Accessibility on Dynamic Badges
**Learning:** Discovered that hidden-by-default dynamic badge elements (like the `error-badge` in `dashboard.html`) lacked attributes to announce changes to screen readers.
**Action:** When building or enhancing dynamic badging elements, ensure they have both `aria-live="polite"` and `aria-atomic="true"` attributes so screen readers correctly announce dynamic changes.
