...
## 2025-06-13 - Avoid Stringifying Dicts in Prompt Length Calculation
**Learning:** Using `len(str(part))` on dictionary objects in LLM message parts triggers massive memory allocations and latency when the dictionary contains large binary payloads.
**Action:** Use explicit type checking (`isinstance(part, dict)`) to evaluate the length of the `text` field specifically, and avoid stringifying the entire dictionary.
