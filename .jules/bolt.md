...
## 2024-05-24 - Avoiding `len(str(part))` for LLM prompt length calculations
**Learning:** Using `len(str(part))` on dictionaries containing large base64 image strings causes massive memory allocations and latency, similar to text extraction issues.
**Action:** Use explicit type checking to calculate lengths specifically on the `text` field of dictionaries and return 0 for raw byte formats (`bytes`, `bytearray`).
