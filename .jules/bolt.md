...
## 2024-05-24 - Large Binary Allocation Overhead
**Learning:** Blindly stringifying objects in dictionary comprehension or nested list iterations (`str(p) for p in parts`) for performance-critical message parts processing causes a massive O(N) memory allocation hit if dictionaries contain binary objects like `image_url` or `file_data`. This degrades performance substantially.
**Action:** Strictly type check values before iterating or stringifying to extract specifically what's needed (e.g., `p["text"]`) rather than attempting a catch-all cast.
