...
## 2024-05-24 - Avoiding Massive String Allocations in Message Parsers
**Learning:** Blindly calling `str()` on message parts (`dict`s or `list`s) can cause massive string allocations (O(N) overhead) if a dictionary payload contains large binary data encoded as values like `inline_data`, `image_url`, or `file_data`.
**Action:** When extracting text from messages or parts, avoid generic list comprehensions with `str()`. Always check `isinstance(p, (bytes, bytearray))` to skip it, and for dictionaries, explicitly check if it contains large payload keys (`inline_data`, `image_url`, `file_data`) and skip them. Only call `str()` on explicitly known text fields.
