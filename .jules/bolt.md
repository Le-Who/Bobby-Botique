...
## 2025-02-19 - Regex Compilation Overhead
**Learning:** Inline regular expression calls (e.g., `re.sub(pattern, ...)`) compile the pattern on every invocation, causing significant overhead in text processing functions like `markdown_to_html` and `split_text_safe`.
**Action:** Always precompile regular expressions using `re.compile()` at the module level for patterns used repeatedly within functions.
