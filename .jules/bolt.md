...
## 2025-06-16 - Optimizing token prompt length calculation for multi-modal payloads
**Learning:** Blindly evaluating `len(str(part))` for multi-modal payload metrics synchronously converts massive base64 image strings and byte arrays into strings. In large chat histories, this blocks the main thread and spikes memory consumption exponentially.
**Action:** Always implement explicit type checking (`isinstance(p, (bytes, bytearray, Image.Image))`) to skip binary formats, and only extract the `"text"` key from dictionaries rather than using catch-all fallbacks like `len(str(part))` inside length-calculation routines.
