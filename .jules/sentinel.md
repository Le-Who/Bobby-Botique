...
## 2025-03-08 - [Quart ASGI IP Spoofing and Rate Limit Bypass]
**Vulnerability:** The Quart web application running behind a reverse proxy (Northflank) was missing ASGI middleware to parse the `X-Forwarded-For` header. This allowed `request.remote_addr` to incorrectly resolve to the proxy's IP (risking a global rate limit denial of service across all users) or blindly parsing untrusted headers leading to trivial IP spoofing (bypassing brute-force limiters).
**Learning:** Quart does not natively process `X-Forwarded-For` headers safely out of the box. Third-party middlewares like `hypercorn.middleware.ProxyFixMiddleware` may be broken or unavailable in the environment, making it necessary to implement a custom ASGI proxy fix that correctly anchors the client IP using a precise `trusted_hops` offset from the right of the header array.
**Prevention:** Always wrap the core ASGI application object (`quart_app.asgi_app`) with a proxy middleware (e.g., `ASGIProxyFix(..., trusted_hops=1)`) when deployed behind a known reverse proxy, strictly validating and parsing headers rather than leaving it to application code.
