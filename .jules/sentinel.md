...
## 2025-05-25 - [Hash Algorithm Upgrade] Replace MD5 with SHA256\n**Vulnerability:** The application used `hashlib.md5()` in `app/utils/response_tags.py` and `app/middleware/dedup.py` for request deduplication and identifying UI elements. MD5 is cryptographically weak and often flagged by security scanners, making it an unsuitable default even for non-cryptographic purposes where collisions might still be exploited or cause functional issues.\n**Learning:** Relying on weak algorithms like MD5 can trip automated scanners and establish bad security habits across the repository. Modern processors handle SHA256 just as efficiently for small payloads.\n**Prevention:** Establish a project-wide convention to exclusively use `hashlib.sha256()` or stronger algorithms, completely deprecating MD5.
