...
## 2026-05-29 - [Cross-Site Scripting] Dynamic DOM Interpolation
**Vulnerability:** The dashboard template injected JSON API response data directly into the DOM using `innerHTML` and template literals (e.g., `tbody.innerHTML = ... ${r.model_name} ...`) without sanitization, leading to DOM-based XSS.
**Learning:** Frontend JavaScript that modifies `innerHTML` must explicitly escape user-controlled or dynamically retrieved data, even if it originates from a "trusted" internal API, as API data can be polluted.
**Prevention:** Implement and use a lightweight browser-native HTML escaping function (`function esc(s) { const d = document.createElement('div'); d.textContent = s ?? ''; return d.innerHTML; }`) for any variable interpolated into an `innerHTML` string.
