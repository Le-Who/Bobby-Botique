...
## 2024-03-01 - [Rate Limiting Denial of Service Bypass due to Proxy Architecture]
**Vulnerability:** The login rate-limiter for the dashboard in `app/web.py` originally relied directly on `request.remote_addr`. Since this application is deployed behind a reverse proxy/load balancer (indicated by Cloud/Northflank deployment config), `request.remote_addr` resolves to the proxy's IP. This meant a single user failing to log in 5 times could lock out all legitimate users globally by getting the proxy IP banned. Furthermore, naive parsing of `X-Forwarded-For` by trusting the first IP (`split(",")[0]`) introduces an IP spoofing vulnerability, allowing attackers to bypass rate limits entirely.
**Learning:** In cloud-deployed applications, standard connection IPs must be treated as proxy IPs. Any rate-limiting, IP-banning, or brute-force protection must parse forwarded headers (like `X-Forwarded-For`) securely. The secure approach, assuming 1 trusted proxy hop, is to extract the *last* (rightmost) IP added to the `X-Forwarded-For` chain by the trusted proxy, ignoring any user-provided spoofed IPs at the start of the list.
**Prevention:** Always use a helper method to resolve the real client IP securely by inspecting `X-Forwarded-For` and taking the rightmost untrusted IP (e.g., `ips[-1]`) before applying IP-based security mechanisms in web routes.
